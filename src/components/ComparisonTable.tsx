...
export default function ComparisonTable({ 
  ...
  return (
    <div className="w-full mx-auto my-8">
      ...
      >
        {title && (
          <h3 className="text-xl md:text-2xl font-bold mb-4 text-center text-gray-900 dark:text-white">
            {title}
          </h3>
        )}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800">
                <th className="px-4 py-3 text-left font-semibold text-gray-700 dark:text-gray-300 border-b">
                  {language === 'en' ? 'Feature' : 'Funktion'}
                </th>
                <th className="px-4 py-3 text-center font-semibold text-wolt-blue dark:text-wolt-light border-b">
                  Wolt
                </th>
            ...
            <tbody>
              {items.map((item, index) => (
                ...
                >
                  <td className="px-4 py-3 text-left border-b text-gray-800 dark:text-gray-200 font-medium">
                    {item.label}
                  </td>
                  <td className={`px-4 py-3 text-center border-b ${item.woltHighlight === false ? getHighlightColor(colorScheme, true) : item.woltHighlight === true ? getHighlightColor(colorScheme, false) : ''}`}>
                    {typeof item.wolt === 'number' && item.unit 
                      ? `${item.wolt}${item.unit}`
                      : item.wolt}
                  </td>
                  <td className={`px-4 py-3 text-center border-b ${item.justEatHighlight === true ? getHighlightColor(colorScheme, true) : item.justEatHighlight === false ? getHighlightColor(colorScheme, false) : ''}`}>
                    {typeof item.justEat === 'number' && item.unit 
                      ? `${item.justEat}${item.unit}`
                      : item.justEat}
                  </td>
                  <td className={`px-4 py-3 text-center border-b ${item.directHighlight === true ? getHighlightColor(colorScheme, true) : item.directHighlight === false ? getHighlightColor(colorScheme, false) : ''}`}>
